...
| Recipe Generation Config | MVP | Meal count, unified max time, meal type, scope mode, allow_ai_generation toggle, match_strictness, expired-item resolution, pantry item checklist (select/deselect which items are available for this recipe). On mobile the meal-count, max-time and meal-type cards stack in a single column (3 columns from the `md` breakpoint up). |
